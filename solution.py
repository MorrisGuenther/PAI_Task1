...
class Model(object):
    ...
    def predict(self, x: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        x_transformed = self.feature_map_nystroem.transform(x_standardized)
        ...
        predictions = \
        (gp_mean < 35.5) * ((gp_mean + gp_std) < 35.5) * (gp_mean - gp_std) + \
        (gp_mean < 35.5) * ((gp_mean + gp_std) > 35.5) * (gp_mean + gp_std) + \
        (gp_mean > 35.5) * ((gp_mean - gp_std) < 35.5) * gp_mean + \
        (gp_mean > 35.5) * ((gp_mean - gp_std) > 35.5) * (gp_mean - gp_std)
    ...
    def fit_model(self, train_x: np.ndarray, train_y: np.ndarray):
        ...
        kernel = DotProduct() + Matern() + RBF()
        self.feature_map_nystroem = Nystroem(kernel=kernel, n_components=500)

        train_x_standardized = self.scaler.fit_transform(train_x)
        train_x_transformed = self.feature_map_nystroem.fit_transform(train_x_standardized)
...
def perform_extended_evaluation(model: Model, results_dir: str):
    """
    Visualizes the predictions of a fitted model.
    :param model: Fitted model to be visualized
    :param results_dir: Directory in which the visualizations will be stored
    """
    ...
    # Save figure to pdf
    figure_path = os.path.join(results_dir, 'extended_evaluation.pdf')
...
def main():
    raise RuntimeError(
        ...
        'The checker always calls the methods of your Model class directly.\n'
    ...
    if EXTENDED_EVALUATION:
        perform_extended_evaluation(model, os.curdir)
